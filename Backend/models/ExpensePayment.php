...
class ExpensePayment extends ExpensePayment_Skeleton
{
    ...
    static function read($data)
    {
        ...
        if ($id) {
            ...
            $stmt = $conn->query("SELECT payment_accounts.id as id,
        expense_payments.account_id,
        payment_accounts.name,
        expense_payments.amount,
        expense_payments.created_on,
        expense_payments.is_deleted,
        users.username AS created_by
    FROM expense_payments,users,payment_accounts
    WHERE expense_payments.account_id = payment_accounts.id 
    AND expense_payments.created_by = users.id
    AND expense_payments.is_deleted = 0");
